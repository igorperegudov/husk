...
| Field             | Default          | Notes                                                              |
| ----------------- | ---------------- | ------------------------------------------------------------------ |
| `mode: llm`       | -                | Selects the LLM executor. Implied when `tools` is set.             |
...
Any frontmatter key HUSK does not recognize is preserved but ignored. That is what makes existing Agent Skills and elisym manifests load unchanged - marketplace fields like `price`, `token`, `capabilities`, and `tools` are simply skipped. See [Compatibility](/reference/compatibility).
