...
```txt
Read https://husk.elisym.network and scaffold a HUSK skill in Python that takes a city name on stdin and prints the current weather as JSON, then serve it with `husk serve` and show me a working curl.
```
